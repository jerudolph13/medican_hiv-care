...
for (cutpoint in cutpoint1){
  ...
  for (analysis in analysis1){
    ...
    # Re-weight to be among those alive
    res <- res %>% 
      mutate(est1 = est1/(1 - est5),
             est2 = est2/(1 - est5),
             est3 = est3/(1 - est5),
             est4 = est4/(1 - est5))
    ...
    risk_year <- function(dat, years_of_interest) {
      ...
      mutate(risk_nrna = round(est4*100, 1),
        ...
        select(year, risk_nrna, risk_rna, risk_nra, risk_ra)
      }
    ...
    # Export csv
    if (analysis=="overall") {
      name <- paste0("../tables/mpr", cutpoint, "/table_mpr", cutpoint, "_wt_nodeath.csv")
      } else {
        name <- paste0("../tables/mpr", cutpoint, "/table_mpr", cutpoint, "_wt_", analysis,"_nodeath.csv")
        }
