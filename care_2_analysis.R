...
if (mpr_limit>0) {
  ...
  mutate(n_cond = myocard_infarct + cong_heart_fail + periph_vas_dis + cereb_vas_dis + 
    ...
    select(bene_id, end_date, dob, dod, int, int_start, start, int_end, end, state, drop, 
           age_base, sex, race, first_elig_state, n_cond, enroll_periodt) 
...
write_csv(res, paste0("./care-cascade/results/mpr", mpr_limit, "/risk_crude.csv"))
...
write_csv(est, paste0("./care-cascade/results/mpr", mpr_limit, "/risk_wt.csv"))
