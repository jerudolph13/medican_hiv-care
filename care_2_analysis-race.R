  ...
  dat <- read_csv("./care-cascade/data/care-cascade_data.csv", 
    ...
    mutate(n_cond = myocard_infarct + cong_heart_fail + periph_vas_dis + cereb_vas_dis + 
           ...
           enroll_period = factor(case_when(yr_base %in% 2001:2005 ~ 1,
                                            ...
                                            T ~ 3)),
           race = factor(race, levels=1:4, labels=c("white", "black", "hisp", "other")),
           state = case_when(
...
# Run analysis by race ----------------------------------------------------

for (s in c("white", "black", "hisp")) {
  
  new.dat <- filter(dat, race==s)
...
# Weighted multistate model -----------------------------------------------
  ...
  boot_rep <- function(r) {
    ...
    mod_drop <- glm(I(drop==0) ~ bs(int, df=5) + bs(age_base, df=3) + sex + first_elig_state + 
