...
generate_figure <- function(analysis, mpr_limit, subgroup = NULL) {
    if (is.null(subgroup)) {
        ...
        filename <- paste0("../figures/mpr", mpr_limit, "/fig_", analysis, "_nodeath.jpeg")
    } else {
        data_path <- paste0("../results/mpr", mpr_limit, "/risk_", analysis, "_", subgroup, ".csv")
        filename <- paste0("../figures/mpr", mpr_limit, "/fig_", analysis, "_", subgroup, "_nodeath.jpeg")
    }
    ...
    p <- ggplot(data=res3) +
        ...
        # geom_area(aes(x = res2$date, y = res2$est12345, fill = "Dead")) +
        geom_area(aes(x = date, y = est1234_s, fill = "Not retained; Not ART adherent")) +
        geom_area(aes(x = date, y = est123_s, fill = "Retained; Not ART adherent")) +
        geom_area(aes(x = date, y = est13_s, fill = "Not retained; ART adherent")) +
        geom_area(aes(x = date, y = est1_s, fill = "Retained; ART adherent")) +
...
for (mpr_limit in c(0, 70, 80, 90)) {
  ...
  theme(legend.position = "bottom")
    ...
    ggsave(
        filename = paste0("../figures/mpr", mpr_limit, "_fig_by_region_", analysis, "_nodeath.jpeg"),
        plot = fig_by_region,
  ...
  theme(legend.position = "bottom")
    ...
    ggsave(
        filename = paste0("../figures/mpr", mpr_limit, "_fig_by_sex_", analysis, "_nodeath.jpeg"),
        plot = fig_by_sex,
  ...
  theme(legend.position = "bottom")
    ...
    ggsave(
        filename = paste0("../figures/mpr", mpr_limit, "_fig_by_race_", analysis, "_nodeath.jpeg"),
        plot = fig_by_race,
    ...
    ggsave(
        filename = paste0("../figures/mpr", mpr_limit, "_fig_main_", analysis, "_nodeath.jpeg"),
        plot = main_fig,
        height = 5, width = 7, units = "in", dpi = 300
    )
}
