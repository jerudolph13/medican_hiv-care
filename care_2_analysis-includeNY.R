...
#
# Purpose: Analyze data (all states end in 2015)
#
...
if (mpr_limit>0) {
  dat <- read_csv("./care-cascade/data/care-cascade_data_sensitivity.csv", 
  ...
  mutate(n_cond = myocard_infarct + cong_heart_fail + periph_vas_dis + cereb_vas_dis + 
    ...
    select(bene_id, end_date, dob, dod, int, int_start, start, int_end, end, state, drop, 
           age_base, sex, race, first_elig_state, n_cond, enroll_period)
...
surv <- survfit(Surv(start, end, as.factor(state)) ~ 1, 
                data=dat, id=bene_id, conf.type="log-log")
...
write_csv(res, paste0("./care-cascade/results/mpr", mpr_limit, "/risk_crude_includeNY.csv"))
...
boot_rep <- function(r) {
  ...
  mod_drop <- glm(I(drop==0) ~ bs(int, df=5) + bs(age_base, df=3) + race + sex + 
                    first_elig_state + n_cond + enroll_period,
...
write_csv(est, paste0("./care-cascade/results/mpr", mpr_limit, "/risk_wt_includeNY.csv"))
